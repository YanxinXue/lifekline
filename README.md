...
4. **在线 AI K 线可选**: 人生 K 线检测到在线配置后会直接调用模型生成报告；没有配置时仍走复制提示词流程。
5. **本地缓存配置**: API Key、Base URL、Model 只保存在当前浏览器 `localStorage`，不上传后端。
6. **人生 K 线独立 Tab**: 可切换到八字命理 K 线流程，用股票 K 线图展示 1-100 岁的人生运势起伏。
7. **AI 深度批断**: 人生 K 线支持性格、事业、财富、婚姻、健康、六亲、发展风水和 Web3 交易风格等多维度报告。
...
右上角模式按钮可配置 OpenAI-compatible API：
...
- 这套配置同时用于黄大仙灵签和人生 K 线。
- 灵签在线配置弹窗中可以切换回本地模式，且不会删除已缓存的配置。

### 人生 K 线

1. **切换 Tab** - 点击顶部“人生K线”。
2. **填写八字信息** - 输入四柱干支和大运信息。
3. **有在线配置时** - 系统直接调用浏览器缓存中的在线 AI 配置，模型返回 JSON 后自动生成报告。
4. **流式生成反馈** - 在线生成使用纯前端 `fetch` 流式读取，生成过程中会显示接收进度。
5. **无在线配置时** - 页面展示完整提示词，可复制到 ChatGPT、Claude、Gemini 等任意 AI。
6. **导入结果** - 手动模式下将 AI 返回的 JSON 数据粘贴回来。
7. **查看 K 线** - 生成完整的人生 K 线图和分析报告。
...
| `components/ImportDataMode.tsx` | 人生 K 线在线生成或复制提示词/导入 JSON 流程 |
| `services/geminiService.ts` | 人生 K 线在线 AI 请求和响应解析 |
