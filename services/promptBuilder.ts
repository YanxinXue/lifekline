...
- 大运交替年是重要转折点，应体现评分跳变
- 100 年整体不能在某个分数区间长期横盘。例如虚岁 30 岁之后不能连续大量停留在 80-85 附近；每一步大运内部也应有流年高低变化。
- 如果某一步大运整体较吉，也要区分该十年内的强弱年、转折年和回撤年，不能只给接近相同的 K 线。
- 任意连续 10 年内，close 最大值与最小值差值不得小于 12；任意连续 20 年内，high 最大值与最低 low 的差值不得小于 25。
- 任意连续 8 年内，不得出现超过 3 组完全相同的 open/close/high/low。
- 即使进入稳定好运，也要用小牛市、回撤、整理、再突破的结构表达，而不是一条高位水平线。
- 输出前自检：如果后半生 K 线肉眼看起来像横盘，请重新生成 chartPoints，直到 100 年走势具有清晰波段。
